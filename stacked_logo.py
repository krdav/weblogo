...
def make_logo(pi, pe, index, pf=None):
    ...
    for v, l in zip(var, ll):
        ...
        with open(dirname+profile, 'w') as fh_out:
            fh_out.write(','.join(map(str, v)))
        logoname = 'logo_p{}_{}.eps'.format(l, index)
        logos.append(logoname)
        if dirname != '':
            cmd = 'cd {}; {} < {} --weight 0 --units probability --stacks-per-line 149 --errorbars False --fineprint "" --alphabet ACDEFGHIKLMNPQRSTVWY > {}; cd ..'.format(dirname, WEBLOGO_EXE, profile, logoname)
        else:
    ...
    #os.system(cmd)
    with open(dirname+logos[0]) as fh:
        first = fh.readlines()
    logolines = list()
    with open(dirname+logos[1]) as fh:
        flag = 0
        for line in fh:
    ...
    if pf is not None:
        with open(dirname+logos[2]) as fh:
            flag = 0
            for line in fh:
                if line.startswith('StartLine'):
                    logolines.append(line)
    ...
    logoname = 'logo_stacked_{}.eps'.format(index)  # merged logo
    with open(dirname+logoname, 'w') as fh_out:
        for line in stacked_logo:
            fh_out.write(line)
    if dirname != '':
        cmd = 'cd {}; epstopdf {}; cd ..'.format(dirname, logoname)
    else:
